...
if(!empty($img)){
$add=$con->query("INSERT INTO `admin_login`(`UserName`,`Password`,`Phone`,`Img`,`Status`)
 VALUES('$username','$password','$phone','$img','$role')");
}else{
  $add=$con->query("INSERT INTO `admin_login`(`UserName`,`Password`,`Phone`,`Status`)
...
<body>
 ...
 if(isset($_GET['delete'])){
       $ID=$_GET['delete'];
      $data1=$con->query("SELECT * FROM `admin_login` WHERE ID='$ID'");
      $data1=$data1->fetch(PDO::FETCH_ASSOC);
    if($_SESSION['admin'] !=$_GET['delete']){   
     $deleteAdmin= $con->query("DELETE FROM `admin_login` WHERE ID='$ID'"); 
    ...
    <div class="main-content">
      ...
      <table id="files_list" class="table table-striped dt-responsive  " style="width:100%">
        ...
        <tbody>
          ...
          $get_admin_data=$con->query("SELECT * FROM `admin_login`");
